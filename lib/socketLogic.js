...
var getDeviceWithoutFromDevice = require('./getDevices').getDeviceWithoutFromDevice;
...
function socketLogic (socket, secure, skynet){
  ...
  socket.on('devices', throttler.throttle(function (data, fn) {
    fn = fn || _.noop
    if (config.knotInstanceType === 'cloud' && data.gateways &&
        socket.skynetDevice && socket.skynetDevice.uuid) {
      data.query = {
        "owner": socket.skynetDevice.uuid,
        "gateways": data.gateways
      };
      knotGatewayManager.getGatewayDevices(data, fn);
    }
    else {
      getDevice(socket, function (err, device) {
        skynet.sendActivity(getActivity('devices', socket, device));
        if (err) { return; }
        var query = {"owner": device.uuid};
        getDeviceWithoutFromDevice(query, true, function (error, results) {
          results.fromUuid = device.uuid;
          results.from = _.pick(device, config.preservedDeviceProperties);
          logEvent(403, results);

          if (results.error) {
            meshbluEventEmitter.emit('devices-error', {
              request: data,
            ...
            meshbluEventEmitter.emit('devices', {
              request: data,
              fromUuid: device.uuid
            });
          }
