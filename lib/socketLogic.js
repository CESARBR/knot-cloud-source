...
function socketLogic (socket, secure, skynet){
  ...
  socket.on('identity', throttler.throttle(function (data) {
    ...
    updateSocketId(data, function(auth){
      ...
      if (config.parentConnection.uuid) {
        ...
        knotGatewayManager.addMapping(mapping, function (result) {
          if (parentConnection && knotGatewayManager.getDeviceMappings() && knotGatewayManager.getDeviceMappings().length > 0) {
  ...
  socket.on('disconnect', function (data) {
    ...
    getDevice(socket, function(err, device){
      ...
      if (socket.auto_set_online) {
       ...
       knotGatewayManager.removeGateway(socket, function () {
  ...
  socket.on('devices', throttler.throttle(function (data, fn) {
    fn = fn || _.noop
    if (data.gateway && socket.skynetDevice && socket.skynetDevice.uuid) {
      data.query = { "owner": socket.skynetDevice.uuid };
  ...
  socket.on('update', throttler.throttle(function (data, fn) {
    ...
    if (data.gateway) {
      knotGatewayManager.updateGatewayDevices(data, function (device) {
        fn(device);
      });
    }
    else {
  ...
  socket.on('addDeviceMapping', throttler.throttle(function(data, fn){
    ...
    knotGatewayManager.addMapping(data,function(result){
      console.log(result);
    });
  }));

  socket.on('updateGatewayDeviceMapping', throttler.throttle(function(data, fn){
    knotGatewayManager.updateGatewayDeviceMapping(data,function(result){
      console.log(result);
    });
  }));
