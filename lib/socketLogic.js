...
function socketLogic (socket, secure, skynet){
  ...
  socket.on('identity', throttler.throttle(function (data) {
    ...
    updateSocketId(data, function(auth){
      ...
      knotGatewayManager.isGateway(socket, function (result) {
        if (result)
  ...
  socket.on('status', throttler.throttle(function (fn) {
    ...
    //ParentConnection bidirectional communication example
    //if (parentConnection) {
    //    parentConnection.socket.emit('status', function (data, fn) {
    //  });
    //}
