    ...
    authorizeRequest(req, res, function(fromDevice){
      ...
      getDevices(fromDevice, req.query, false, function(error, data){
          ...
          if(data.error){
            ...
            knotGatewayManager.gateways.forEach(function (gateway, outerIndex) {
              gateway.gatewaySocket.emit('getDevices', { request: req.query, fromUuid: gateway.gatewayDevice.uuid }, function (result) {
                result.devices.forEach(function (device, innerIndex) {
                  data.devices.push(device);
                });

                if (outerIndex === knotGatewayManager.gateways.length - 1) {
                  res.json(data);
                }
              });
            })
      ...
      if (message.target) {
        knotGatewayManager.getMapping(message.target, function (route) {
          if (route) {
            ...
            res.json({ error: 'Invalid Message Target' });
          }
        });
      }
      else {
