...
function setupHttpRoutes(server, skynet){
  ...
  server.post('/devices', function (req, res) {
    ...
    if (config.knotInstanceType === 'gateway' && (!_.isEmpty(req.body.uuid) ||
        !_.isEmpty(req.body.token))) {
        getDevice(req.body.uuid, (err, device) => {
          if (err) {
            if (err.error.code === 404) {
              register(req.merged_params, function (error, device) {
                if (error) {
                  meshbluEventEmitter.emit('register-error', { request: req.body, error: error.message });
                  return res.status(500).json(error.message);
                } else {
                  meshbluEventEmitter.emit('register', { request: req.merged_params });
                  res.status(201).json(device);
                }
              }, { uuid : req.body.uuid , token: req.body.token });
            }
          } else if (device) {
            meshbluEventEmitter.emit('existent-device-error', { request: req.body });
            return res.status(403).json({ code: 403, message: 'Device exists' });
          }
        });
    } else {
