  ...
  server.get('/mydevices', function(req, res){
    ...
    authorizeRequest(req, res, function(fromDevice){
      ...
      getDevices(fromDevice, query, true, function(error, data){
        ...
        if (knotGatewayManager.gateways && knotGatewayManager.gateways.length > 0) {
          knotGatewayManager.getGatewayDevices(req, function (result) {
            if (result)
...
/*
  server.post('/mydevices/:uuid', function (req, res) {
    skynet.sendActivity(getActivity('devices', req));
    req.merged_params.ipAddress = req.merged_params.ipAddress || getIP(req);
    var payload = {
      body:req.body,
      gateway:req.params.uuid,
    };
    knotGatewayManager.createGatewayDevices(payload, function (result) {
       res.json(result);
    });
  });

  server.put('/mydevices', function (req, res) {
    skynet.sendActivity(getActivity('devices', req));
    req.merged_params.ipAddress = req.merged_params.ipAddress || getIP(req);
    knotGatewayManager.updateGatewayDevices(req, function (result) {
       res.json(result);
    })
  });
*/
