...
var DATE_ARGS_FORMAT, DAY_ARGS_FORMAT, INVALID_ARGS_FORMAT, TIME_ARGS_FORMAT, UNKNOWN_OPERATOR_ERROR, UNKNOWN_PARAMETER_ERROR, UNKNOWN_TYPE_ERROR, _, evaluate, moment, recursive_evaluate, restrictions, strToMomentDate, util;
...
restrictions = {
  ...
  date: function(value) {
    ...
    if (!value.match(/^\s*\d{1,2}(?:\s*\/\s*\d{1,2}(?:\s*\/\s*\d{1,4})?)?(?:\s*-\s*\d{1,2}(?:\s*\/\s*\d{1,2}(?:\s*\/\s*\d{1,4})?)?)?\s*$/)) {
